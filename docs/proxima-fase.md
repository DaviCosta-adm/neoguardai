...
Próximos refinamentos possíveis: colar `RESEND_API_KEY` no Coolify e agendar
o cron diário (comando em `/dashboard/operacao`).
...
Painel de operação (status Resend/cron/Stripe + testes): ✅
(`/dashboard/operacao`)
